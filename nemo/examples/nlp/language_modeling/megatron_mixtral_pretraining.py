...
from nemo.collections.nlp.models.language_modeling.megatron.mixtral_model import MixtralModel
...
class MegatronMixtralModel(MegatronGPTModel):
    """Megatron Mixtral Model."""

    def __init__(self, cfg: DictConfig, trainer: Trainer):
        super().__init__(cfg, trainer)

    def _build_tokenizer(self):
        if self.cfg.tokenizer.get('vocab_size') is not None:
            return

    def _build_vocab(self):
        self.padded_vocab_size = self._vocab_size_with_padding(
            orig_vocab_size=self.cfg.tokenizer.get('vocab_size') or self.tokenizer.vocab_size,
    ...
    def model_provider_func(self, pre_process, post_process):
        ...
        model = MixtralModel(
            ...
            normalize_input=self.cfg.get('normalize_input', False),
            num_moe_experts=self.cfg.get('num_moe_experts', 1),
            num_experts_per_tok=self.cfg.get('num_experts_per_tok', 2),
            moe_aux_loss_coeff=self.cfg.get('moe_aux_loss_coeff', 0.02),
    ...
    def build_train_valid_test_datasets(self):
        logging.info('Building HF datasets.')

        ds = load_from_disk(self.cfg.data.hf_dataset_path)
        if isinstance(ds, DatasetDict):
            self._train_ds = ds["train"]
            if ds.get("validation"):
                self._validation_ds = ds["validation"]
        else:
            self._train_ds = ds

        if self._train_ds is not None:
            self._train_ds = self._train_ds.with_format("torch")
            logging.info(f'Length of train dataset: {len(self._train_ds)}')
        if hasattr(self, '_validation_ds'):
            self._validation_ds = self._validation_ds.with_format("torch")
            logging.info(f'Length of validation dataset: {len(self._validation_ds)}')
        logging.info('Finished building HF datasets.')

        if hasattr(self, '_validation_ds'):
            return self._train_ds, self._validation_ds, None
        else:
            return self._train_ds, None, None
...
def main(cfg) -> None:
    ...
    model = MegatronMixtralModel(cfg.model, trainer)
    trainer.fit(model)
