...
def convert_checkpoint(p, args, config):
    ...
    TP = args.tp_degree
    PP = args.pp_degree
    model_paths = [f"{args.path_to_checkpoint}/pytorch_model-{x:05d}-of-{args.num_shards:05d}.bin" for x in
                   pp_to_bin[str(p)]]
    model_llama = {}
        ...
        for k, v in model_llama.items():
            ...
            if br_key in k:
                ...
                if "query" in (key):
                    ...
                    hidden_size_per_head = config.get("head_dim") or (config["hidden_size"] // heads)
