...
try:
    from apex.normalization.fused_layer_norm import fused_rms_norm_affine, fused_rms_norm_xla, manual_rms_norm
...
if HAVE_APEX:
    class RMSNorm1P(MixedFusedRMSNorm):
        ...
        def forward(self, input):
            if not input.is_cuda:
                return manual_rms_norm(input, self.normalized_shape, self.weight + 1, self.eps)

            if self.elementwise_affine:
                return fused_rms_norm_affine(input, self.weight + 1, self.normalized_shape, self.eps)
            else:
                return fused_rms_norm_xla(input, self.normalized_shape, self.eps)
else:
    class RMSNorm1P(torch.nn.Module):
        def __init__(self, *args, **kwargs):
            raise NotImplementedError('RMSNorm1P available only with apex installed')
...
class ParallelMLP(MegatronModule, adapter_mixins.AdapterModuleMixin):
    ...
    def __init__(
        ...
        if activation in ['geglu', 'reglu', 'swiglu', 'gelu']:
        ...
        self.gelu_activation_family = activation in ['gelu']
    ...
    def forward(self, hidden_states):
        ...
        if self.glu_activation_family or self.gelu_activation_family:
        ...
        if self.bias_activation_fusion:
            if self.activation == 'gelu':
                ...
                (self.glu_activation_family or self.gelu_activation_family)
                and not self.bias_activation_fusion
        ):
...
class TopKRouter(MegatronModule):
    ...
    def __init__(
        ...
        # Initialize the gate.
        self.gate = tensor_parallel.RowParallelLinear(
            hidden_size,
            num_moe_experts,
        ...
        def switch_load_balancing_loss_func(gates, mask, moe_aux_loss_coeff):
            ...
            selection_mean = mask.float().mean(dim=0) / num_experts_per_tok
    ...
    def forward(self, input: torch.Tensor):
        ...
        """
        logits, _ = self.gate(input)
        logits = logits.view(-1, self.num_moe_experts)
    ...
    def aux_loss_load_balancing(self, logits: torch.Tensor):
        ...
        """
        top_logits, indices = self._topk(logits, k=self.topk, dim=1)
    ...
    def apply_aux_loss(
        ...
        mask = self._one_hot(
            ...
            num_classes=self.num_moe_experts,
        ).sum(dim=1)
        aux_loss = loss_func(probs, mask, self.moe_aux_loss_coeff)
    ...
    def _one_hot(self, indices, num_classes):
        # WORKAROUND: At compile time, random values are inserted, so the compile fails if torch.nn.functional.one_hot is used.
        # If the maximum value of indices is greater than num_classes, that index is considered 0.
        one_hot = torch.zeros(
            *indices.shape,
            num_classes,
    ...
    def _topk(self, input, k, dim=-1):
        # WORKAROUND: NeuronX does not support torch.topk.
        topk_values = []
        topk_indices = []
        for _ in range(k):
            max_value, max_index = torch.max(input, dim)
            topk_values.append(max_value)
            topk_indices.append(max_index)

            input = input.scatter(dim, max_index.unsqueeze(dim), float('-inf'))

        topk_values = torch.stack(topk_values, dim)
        topk_indices = torch.stack(topk_indices, dim)

        return topk_values, topk_indices


class SparseMoeBlock(MegatronModule):
    """Sparse MoE Block"""
    ...
    def forward(self, hidden_states):
        # hidden_states: [seq_len, bs, hs]
        # scores, indices: [seq_len, topk]
        scores, indices = self.router(hidden_states)

        expert_num = 0
        local_scores = torch.where(indices == expert_num, scores, 0.0).sum(dim=1)
        output, output_bias = self.experts[expert_num](hidden_states)
        if output_bias is not None:
            output_bias = output_bias.expand_as(output)

        output_total = torch.zeros_like(hidden_states, dtype=output.dtype)
        output_bias_total = (
            torch.zeros_like(hidden_states, dtype=output_bias.dtype)
            if output_bias is not None else None
        )

        output_total += local_scores[:, None, None] * output
        if output_bias_total is not None:
            output_bias_total += local_scores[:, None, None] * output_bias
...
class ParallelTransformerLayer_(MegatronModule, adapter_mixins.AdapterModuleMixin):
    ...
    def __init__(
        ...
        if normalization not in ['layernorm', 'layernorm1p', 'rmsnorm', 'rmsnorm1p']:
        ...
        if self.layer_type != LayerType.retrieval_decoder_after_self_attn:
            ...
            elif normalization == 'rmsnorm1p':
                self.input_layernorm = RMSNorm1P(hidden_size, layernorm_epsilon,
                                                         sequence_parallel_enabled=sequence_parallel)
            else:
            ...
            if self.layer_type != LayerType.decoder_pre_mlp or self.transformer_block_type != 'post_ln':
                ...
                if normalization == 'layernorm':
                    ...
                    self.post_attention_layernorm = RMSNorm1P(hidden_size, layernorm_epsilon,
                                                         sequence_parallel_enabled=sequence_parallel)
                else:
        ...
        if num_moe_experts > 1:
            logging.trace("In ParallelTransfomerLayer() create SparseMoeBlock ....", trace_type="recovery_time")
    ...
    def __init__(
        ...
        if self.post_process and self.transformer_block_type != 'post_ln':
            ...
            if normalization == 'layernorm':
                ...
                self.final_layernorm = RMSNorm1P(hidden_size, layernorm_epsilon,
                                                         sequence_parallel_enabled=sequence_parallel)
            else:
...
def get_mixtral_language_model(
    hidden_size,
    ffn_hidden_size,
    num_layers,
